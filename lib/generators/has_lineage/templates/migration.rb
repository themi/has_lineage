class <%= migration_class_name %> < ActiveRecord::Migration
  def change
    ...
    add_column :<%= table_name %>, :branch_id, :integer               # Remove if only one tree hierarchy needed
    ...
    add_index :<%= table_name %>, :branch_id                          # Remove if only one tree hierarchy needed
  end
end
